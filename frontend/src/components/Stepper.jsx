        ...
        {!isCompleted && (
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              {currentStep !== 1 && (
                <button onClick={handleBack} className="back-button" {...backButtonProps} type="button">
                  {backButtonText}
                </button>
              )}
              <button onClick={isLastStep ? handleComplete : handleNext} className="next-button" {...nextButtonProps} type="button">
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#f5f5f7', color: '#7a7a7a' },
          active: { scale: 1, backgroundColor: '#0066cc', color: '#0066cc' },
          complete: { scale: 1, backgroundColor: '#0066cc', color: '#0066cc' }
        }}
